...
/* harmony import */ var _jupyterlite_application__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_jupyterlite_application__WEBPACK_IMPORTED_MODULE_4__);
...
const notebookPlugin = {
    ...
    activate: (app, tracker, toolbarRegistry, notebookFactory, settingRegistry, translator, router) => {
        ...
        const flushVfsCaches = () => {
            tracker.forEach(w => {
                ...
                if (sessionStorage.getItem(`vfs-cache:${path}`) !== null) {
                    try { sessionStorage.setItem(`vfs-cache:${path}`, JSON.stringify(w.context.model.toJSON())); } catch { /* ignore quota errors */ }
                }
            });
        };

        // ── Cross-page persistent logger ──────────────────────────────────────────
        // Survives page redirects so we can trace both the file2 page AND the file1
        // page in a single localStorage dump after the dialog appears.
        // To read: JSON.parse(localStorage.getItem('ck-log') || '[]')
        // To clear: localStorage.removeItem('ck-log')
        const ck_log = (...args) => {
            const msg = args.map(a => { try { return typeof a === 'object' ? JSON.stringify(a) : String(a); } catch(e) { return '?'; } }).join(' ');
            console.log('[CK]', msg);
            try {
                const logs = JSON.parse(localStorage.getItem('ck-log') || '[]');
                logs.push({ t: new Date().toISOString(), url: location.search.slice(0,60), msg });
                if (logs.length > 300) logs.splice(0, logs.length - 300);
                localStorage.setItem('ck-log', JSON.stringify(logs));
            } catch(e) {}
        };

        // Intercept ALL commands.execute calls to see who triggers docmanager:open
        try {
            const _origExec = commands.execute.bind(commands);
            commands.execute = (id, args) => {
                if (typeof id === 'string' && id.startsWith('docmanager')) {
                    const stack = new Error().stack.split('\n').slice(1,4).join(' | ');
                    ck_log('cmd.execute:', id, JSON.stringify(args||{}), '<<', stack);
                }
                return _origExec(id, args);
            };
        } catch(e) { console.warn('CK: could not intercept commands.execute', e); }

        ck_log('=== activate page:', location.search.slice(0,80), '===');
        // ─────────────────────────────────────────────────────────────────────────
        ...
        if (!fsaSupported && !sessionStorage.getItem('ck-fsa-notice')) {
            ...
            setTimeout(() => {
                _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_6__.Notification.info(
        ...
        const openUploadedNotebook = async (id) => {
            ...
            try {
                ...
                ck_log('[openUploaded] BEFORE open:', filename, 'tracker:', preOpenWidgets);
                ...
                ck_log('[openUploaded] AFTER open:', filename, 'tracker:', postOpenWidgets);
                ...
                if (!fileHandle && !sourceUrl) {
                    try { sessionStorage.setItem(`vfs-cache:${filename}`, JSON.stringify(content)); } catch { /* ignore quota errors */ }
        ...
        const openRecentNotebook = async (nb) => {
            ...
            if (nb.type === 'vfs' && nb.path) {
                ...
                if (!cached) {
                    _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_6__.Notification.warning(
            ...
            if (nb.type === 'file' && nb.handleKey) {
                ...
                try {
                    ...
                    ck_log('[openRecent] tracker at switch time:', JSON.stringify(trackerPaths), 'existingId:', existingId);
                    if (existingId) {
                        ck_log('[openRecent] FAST PATH — clearing dirty + activating', handle.name);
                        tracker.forEach(w => { w.context.model.dirty = false; });
                        app.shell.activateById(existingId);
                        notebookSourceUrl = null;
                    ...
                    ck_log('[openRecent] queryPermission for', handle.name);
                    let perm = await fsa.queryPermission({ mode: 'read' });
                    ck_log('[openRecent] queryPermission result:', perm, 'for', handle.name);
                    if (perm !== 'granted') {
                        ck_log('[openRecent] requesting read permission for', handle.name);
                        perm = await fsa.requestPermission({ mode: 'read' });
                        ck_log('[openRecent] requestPermission result:', perm, 'for', handle.name);
                    }
                    if (perm !== 'granted') {
                    ...
                    const preRedirectWidgets = [];
                    tracker.forEach(w => { preRedirectWidgets.push(`${w.context.path}(dirty=${w.context.model.dirty})`); });
                    ck_log('[openRecent] REDIRECTING to', handle.name, 'tracker:', preRedirectWidgets);
                    flushVfsCaches();
                    _ckIntentionalNav = true;
                    tracker.forEach(w => { w.context.model.dirty = false; });
                    window.location.href = target.toString();
        ...
        commands.addCommand(_commands__WEBPACK_IMPORTED_MODULE_11__.Commands.saveToFile, {
            ...
            execute: async () => {
                ...
                try {
                    ...
                    await (0,_filesystem__WEBPACK_IMPORTED_MODULE_14__.storeRecentHandle)(recentKey, handle);
                    (0,_recents__WEBPACK_IMPORTED_MODULE_15__.addRecentNotebook)({ label: handle.name, type: 'file', handleKey: recentKey });
                    // Clear dirty so subsequent navigation doesn't trigger a spurious save warning
                    await panel.context.save();
        ...
        commands.addCommand(_commands__WEBPACK_IMPORTED_MODULE_11__.Commands.closeNotebook, {
            ...
            execute: async () => {
                ...
                if (!fsaSupported && panel) {
                    ...
                    const _currentCells = JSON.stringify(panel.context.model.toJSON().cells ?? []);
            ...
            execute: async () => {
                ...
                const githubEnterHandler = (e) => {
                    if (e.key === 'Enter' && e.target instanceof HTMLInputElement && e.target.classList.contains('je-GitHubBrowser-input')) {
                        ...
                        const openBtn = e.target.closest('.je-GitHubBrowser')?.querySelector('.je-GitHubBrowser-browse-btn');
        ...
        tracker.widgetAdded.connect(async (_, panel) => {
            ...
            panel.context.model.stateChanged.connect(() => {
                ...
                const currentCells = JSON.stringify(panel.context.model.toJSON().cells ?? []);
                if (currentCells === _initialCells) {
                    ck_log('[CK] spurious dirty (cells unchanged) → clearing', panel.context.path);
                    panel.context.model.dirty = false;
                }
            });
        ...
        // Capture-phase: runs BEFORE JupyterLab's bubble-phase dirty check.
        // Clears model dirty so JupyterLab's handler won't set event.returnValue.
        window.addEventListener('beforeunload', (event) => {
            ck_log('[beforeunload] capture: intentional=', _ckIntentionalNav);
            if (_ckIntentionalNav) {
                tracker.forEach(w => {
                    ck_log('[beforeunload] clearing dirty for', w.context.path, 'was:', w.context.model.dirty);
                    w.context.model.dirty = false;
                });
            }
        }, true);
        // Bubble-phase: runs AFTER JupyterLab's bubble-phase handler (ours registered later → runs last).
        // Belt-and-suspenders: if JupyterLab still set event.returnValue, clear it now.
        window.addEventListener('beforeunload', (event) => {
            ck_log('[beforeunload] bubble: intentional=', _ckIntentionalNav, 'returnValue=', event.returnValue);
            if (_ckIntentionalNav) {
                event.returnValue = '';
            }
        });
        ...
        toolbarRegistry.addFactory('Notebook', 'upload', () => new _ui_components_OpenDropdownButton__WEBPACK_IMPORTED_MODULE_0__.OpenDropdownButton(commands, () => {
            ...
            void commands.execute(_commands__WEBPACK_IMPORTED_MODULE_7__.Commands.saveNotebookCommand);
        }, () => !!(0,_filesystem__WEBPACK_IMPORTED_MODULE_14__.getCurrentFileHandle)() && !!tracker.currentWidget?.context.model.dirty, () => {
...
//# sourceMappingURL=lib_index_js.14021a1f11ee803c4b76.js.map