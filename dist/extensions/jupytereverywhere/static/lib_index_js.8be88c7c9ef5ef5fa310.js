...
function renderMarkdown(src) {
    ...
    const raw = marked__WEBPACK_IMPORTED_MODULE_1__.marked.parse(out, { async: false });
    const restored = restoreMath(raw, stash);
    return dompurify__WEBPACK_IMPORTED_MODULE_2__["default"].sanitize(restored, { ADD_TAGS: ['mjx-container'] });
}
function renderOutput(output) {
    ...
    if (ot === 'display_data' || ot === 'execute_result') {
        ...
        if (data['image/png']) {
            const b64 = joinLines(data['image/png']).replace(/\s/g, '');
            return `<div class="output"><img src="data:image/png;base64,${b64}"></div>`;
        }
        if (data['image/jpeg']) {
            const b64 = joinLines(data['image/jpeg']).replace(/\s/g, '');
        ...
        if (data['text/html']) {
            const html = joinLines(data['text/html']);
            return `<div class="output">${dompurify__WEBPACK_IMPORTED_MODULE_2__["default"].sanitize(html)}</div>`;
        }
        if (data['text/plain']) {
...
function buildHtml(title, nbJson) {
    var _a;
    let body = '';
    for (const cell of nbJson.cells) {
        ...
        if (cell.cell_type === 'markdown') {
            ...
            for (const out of ((_a = cell.outputs) !== null && _a !== void 0 ? _a : [])) {
...
async function exportNotebookAsPDF(notebook, fileName) {
    ...
    const nbJson = notebook.context.model.toJSON();
