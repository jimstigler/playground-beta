    ...
    activate: (app, tracker) => {
        ...
        commands.addCommand(_commands__WEBPACK_IMPORTED_MODULE_7__.Commands.downloadNotebookCommand, {
            ...
            execute: async (args) => {
                ...
                const result = await (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.showDialog)({
                    ...
                    body,
                    buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog.cancelButton(), _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog.okButton({ label: 'Download' })]
                });
                if (!result.button.accept) {
                    return;
                }
        ...
        tracker.widgetAdded.connect((_, panel) => {
            ...
            const maybeScheduleSaveReminder = () => {
                ...
                if ((0,_notebook_utils__WEBPACK_IMPORTED_MODULE_10__.isNotebookEmpty)(content)) {
                    return;
                }
...
const notebookPlugin = {
    ...
    activate: (app, tracker, toolbarRegistry, notebookFactory, settingRegistry, translator, router) => {
        ...
        const openNewNotebookWindow = (kernelParam) => {
        ...
        const openRecentNotebook = async (nb) => {
            ...
            if (nb.type === 'file' && nb.handleKey) {
                ...
                try {
                    ...
                    // If the widget is already open in the tracker, just switch to it — no reload needed
                    let existingId = null;
                    const trackerPaths = [];
                    tracker.forEach(w => {
                    ...
                    console.log('[openRecent] tracker at switch time:', trackerPaths, 'existingId:', existingId);
                    if (existingId) {
                        console.log('[openRecent] FAST PATH — activating existing widget for', handle.name);
        ...
        commands.addCommand(_commands__WEBPACK_IMPORTED_MODULE_11__.Commands.saveToFile, {
            ...
            execute: async () => {
                ...
                const content = panel.context.model.toJSON();
                const text = JSON.stringify(content, null, 2);
                const fileHandle = (0,_filesystem__WEBPACK_IMPORTED_MODULE_14__.getCurrentFileHandle)();
                if (fileHandle) {
                    try {
                        await (0,_filesystem__WEBPACK_IMPORTED_MODULE_14__.saveToHandle)(fileHandle, text);
                ...
                if (!(0,_filesystem__WEBPACK_IMPORTED_MODULE_14__.isFileSystemAccessSupported)()) {
                    _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_6__.Notification.warning('File saving is not supported in this browser. Use "Download notebook" instead.', { autoClose: 5000 });
                    return;
                }
                ...
                if (notebookSourceUrl !== null) {
                    suggestedName = suggestedName.replace(/\.ipynb$/i, '_new.ipynb');
                }
        ...
        commands.addCommand(_commands__WEBPACK_IMPORTED_MODULE_11__.Commands.openFromGitHub, {
            ...
            execute: async () => {
                ...
                const dialog = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_6__.Dialog({
                    ...
                    body: widget,
                    buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_6__.Dialog.cancelButton({ label: 'Close' })]
                });
        ...
        tracker.widgetAdded.connect(async (_, panel) => {
            ...
            panel.context.model.stateChanged.connect(() => {
                console.log('[stateChanged]', panel.context.path, 'dirty=', panel.context.model.dirty);
            });
...
class OpenDropdownButton extends _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.ToolbarButton {
    constructor(commands, openFromFile, openFromURL, openNewRNotebook, openNewPythonNotebook, downloadNotebook, downloadPDF, openFromGitHub, copyShareLink, isCopyShareLinkEnabled, saveToFile, getRecentItems) {
        ...
        const commandSaveToFile = 'jupytereverywhere:file-save-to-file';
        ...
        if (!commands.hasCommand(commandSaveToFile)) {
            commands.addCommand(commandSaveToFile, {
                label: 'Save to file',
                execute: () => {
                    saveToFile();
                }
        ...
        super({
            ...
            onClick: () => {
                ...
                menu.addItem({ type: 'separator' });
                menu.addItem({ command: commandSaveToFile });
                menu.addItem({ type: 'separator' });
 ...
 */
async function openNotebookContent(parsed, sourceUrl) {
...
//# sourceMappingURL=lib_index_js.31b8fe2ac44cfab1ac4a.js.map