...
/* ── Print / PDF export ──────────────────────────────────────────────────────
   window.print() is called on the current page so that:
     • Markdown is already rendered (not raw) — JupyterLab's JS has run
     • MathJax has already typeset the math — nothing needs to re-run
     • Cross-origin images display freely — browser native print, no canvas
   This CSS overrides Lumino's absolute-position inline styles so the
   notebook flows as a normal document for the print renderer.        ───── */
@media print {
  @page {
  ...
  /* ── Lumino layout reset ─────────────────────────────────────────────── */
  /* Lumino sets position:absolute + explicit dimensions as inline styles.
     !important in an author stylesheet wins over inline styles.           */
  .jp-LabShell,
  .jp-NotebookPanel,
  .jp-Notebook,
  .lm-BoxPanel,
  ...
  /* ── Markdown cells ──────────────────────────────────────────────────── */
  /* JupyterLab has TWO child elements per markdown cell:
       1. .jp-Cell-inputWrapper  — the raw CodeMirror editor
       2. .jp-Cell-outputWrapper — the rendered HTML
     In the live app, JS toggles which one is visible via .jp-mod-rendered.
     For print we always want the rendered output, never raw markdown.    */
  .jp-MarkdownCell .jp-Cell-inputWrapper {
    display: none !important;
  }
  .jp-MarkdownCell .jp-Cell-outputWrapper {
  ...
  /* Remove the left active-cell border accent */
  .jp-Cell.jp-mod-active .jp-Cell-inputWrapper {
    border-left: none !important;
    padding-left: 0 !important;
  }

  /* Keep headings bound to the content after them */
  h1, h2, h3, h4, h5, h6 {
    break-after: avoid;
...
//# sourceMappingURL=style_index_js.361ae6b786ef7204ed52.js.map